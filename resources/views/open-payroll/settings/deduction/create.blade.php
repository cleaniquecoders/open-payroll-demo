            ...
            <div class="card bg-white border-0 shadow-sm rounded">
                <div class="card-header bg-white border-0">
                    @if(isset($type) && $type->id)
                        {{ __('Update Deduction Type') }}
                    @else
                        {{ __('Create New Deduction Type') }}
                    @endif
                </div>
                ...
                <div class="card-body">
                    <form method="POST" 
                        @if(isset($type) && $type->id)
                            action="{{ route('open-payroll.setting.deduction.update', $type->id) }}" 
                        @else
                            action="{{ route('open-payroll.setting.deduction.store') }}" 
                        @endif
                        aria-label="{{ __('Create New Deduction Type') }}">
